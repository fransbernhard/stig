...
export default function EnergyDashboard({ data }) {
  ...
  const totalEnergy = steps.reduce((sum, s) => sum + parseFloat(s.energy_joules || 0), 0)
  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.meta}>
        <p className={styles.ts}>{new Date(data.timestamp).toLocaleString('sv-SE')}</p>
        <a className={styles.link} href={data.workflow_url} target="_blank" rel="noopener">
          Run #{data.run_id} ↗
        </a>
      </div>
      ...
      <div className={styles.totals}>
        <div className={styles.stat}>
          <span className={styles.statVal}>{totalEnergy.toFixed(2)}</span>
          <span className={styles.statLabel}>total joules</span>
        </div>
        <div className={styles.stat}>
          <span className={styles.statVal}>{totalTime.toFixed(1)}</span>
      ...
      <div className={styles.table}>
        <div className={styles.row + ' ' + styles.rowHead}>
          <span>Phase</span><span>Joules</span><span>Seconds</span><span>Avg CPU</span>
        </div>
        {steps.map((s, i) => (
